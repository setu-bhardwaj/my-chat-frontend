...
import { CheckUser } from '../ChatUser';
...
@Component({
  ...
  providers:[SocketServiceService] // making service local to this component
})
export class ChatBoxComponent implements OnInit,CheckUser {
  ...
  constructor(
   ...
   // vcr: ViewContainerRef
    ...
    this.receiverId = Cookie.get('receiverId'); //getting values from cookies
  ...
  ngOnInit() {
    ...
    this.userInfo = this.AppService.gertUserInfoFromLocalStorage();
...
//chat related
//sending message via enter ( event)
...
public userSelectedToChat: any = (id, name) => {
  ...
  this.messageList = []; //when swicthes from one user to other, one user's chat should not be visible to others and making empty.
